...
pub fn get_icon_for_file(path: impl AsRef<Path>) -> Option<u64> {
    ...
    // A path with no final component (e.g. "" or "/") has no icon. Using `?`
    // rather than `unwrap()` keeps this safe on untrusted input from the API.
...
mod test {
    ...
    #[test]
    fn does_not_panic_on_degenerate_paths() {
        // Previously these panicked via `unwrap()`; now they return `None`.
        assert_eq!(get_icon_for_file(""), None);
        assert_eq!(get_icon_for_file("/"), None);
        // A basename with no extension and no filename match.
        assert_eq!(get_icon_for_file("some-unknown-binary"), None);
        assert_eq!(get_icon_for_folder(""), None);
        assert_eq!(get_icon_for_folder("/"), None);
    }
}
